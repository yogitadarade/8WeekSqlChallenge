...
| table_name               | column_name  | data_type                    |
+──────────────────────────+──────────────+──────────────────────────────+
| cleaned_customer_orders  | order_id     | integer                      |
| cleaned_customer_orders  | customer_id  | integer                      |
| cleaned_customer_orders  | pizza_id     | integer                      |
| cleaned_customer_orders  | exclusions   | character varying            |
| cleaned_customer_orders  | extras       | character varying            |
| cleaned_customer_orders  | order_time   | timestamp without time zone  |
+──────────────────────────+──────────────+──────────────────────────────+
...
| table_name             | column_name  | data_type                    |
+────────────────────────+──────────────+──────────────────────────────+
| cleaned_runner_orders  | order_id     | integer                      |
| cleaned_runner_orders  | runner_id    | integer                      |
| cleaned_runner_orders  | pickup_time  | timestamp without time zone  |
| cleaned_runner_orders  | distance     | numeric                      |
| cleaned_runner_orders  | duration     | numeric                      |
| cleaned_runner_orders  | cancellation | character varying            |
+────────────────────────+──────────────+──────────────────────────────+
...
SELECT COUNT (DISTINCT customer_id) AS order_count
FROM cleaned_customer_orders;
...
FROM cleaned_customer_orders AS co
...
INNER JOIN pizza_runner.pizza_names AS pn
   ON co.pizza_id = pn.pizza_id
WHERE EXISTS (
  SELECT 1 FROM cleaned_runner_orders AS ro
...
FROM (
  ...
  FROM cleaned_customer_orders AS co
  INNER JOIN cleaned_runner_orders AS ro
    ON co.order_id = ro.order_id
  WHERE 
    ro.cancellation IS NULL
    OR ro.cancellation NOT IN ('Restaurant Cancellation', 'Customer Cancellation')
...
WITH runner_pickups AS (
  ...
  FROM cleaned_runner_orders AS ro
  INNER JOIN cleaned_customer_orders AS co
